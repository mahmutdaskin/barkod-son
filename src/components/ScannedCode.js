...
const pointGenerator = (points) => (i) =>
  `(${points[i][0].toFixed(0)}, ${points[i][1].toFixed(0)})`;
const lineGenerator = (line) => (i) =>
  `(${line[i].x.toFixed(0)}, ${line[i].y.toFixed(0)})`;
...
const renderLineSegments = (items) => (
  ...
  >
    {items.map((item, i) => (
      <div
        key={i}
        style={{ width: `30%`, textAlign: "center", padding: "0.2rem" }}
      >
        {item}
      </div>
    ))}
...
const renderDirection = (direction) => (
  <span>{direction === -1 ? "forward" : `reverse`}</span>
);
  ...
  return (
    <Card style={{ margin: "0.5em 0.25em 0em" }}>
      <CardHeader
        ...
        title={
          "s:" +
          scannedCode.codeResult.siparis +
          "/plt:PLT-" +
          scannedCode.codeResult.palet +
          "/ad:" +
          scannedCode.codeResult.adet +
          "-" +
